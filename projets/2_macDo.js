function prendreSaCommande(burger_choisi, probleme_grillade, bip_moi_quand_ta_fini) {
    ...
    setTimeout(() => {
        if (!probleme_grillade) {
            ...
            console.log("3. Ah gros on a cramé ton", burger_choisi, "je crois et on peut plus en faire mdrrrr")
    ...
    if (!err) {
        ...
        prendreSaCommande("wrap_New_York_de_malade", false, (err, value) => {
            if (!err) {
                ...
                console.log(err)
                console.log("nan mais là je vais casser mon crâne enft appelle moi le manager (je m'appelle Karen)")
            }
        })
    }
})